                  ...
                  int jproc, double **U, double **V, double **F, double **G, double **P, double *uMax, double *vMax)
{
    ...
    *vMax = 0;
    ...
    if (omg_i != iproc - 1)
    {
        ...
        {
            // U and P have different sizes hence shift in indecies along i
            U[imax_local + 3 - 1][j] = F[imax_local + 3 - 1][j] - (dt / dx * (P[(imax_local + 3 - 1 - 1) + 1][j] -
                                                                              P[(imax_local + 3 - 1 - 1)][j]));
            updateMax(uMax, U[imax_local + 3 - 1][j]);
        }
    }
    ...
    if (omg_j != jproc - 1)
    {
        ...
        for (int i = 1; i < imax_local + 2; ++i)
        {
            ...
            V[i][jmax_local + 3 - 1] =
                    G[i][jmax_local + 3 - 1] -
                    (dt / dy * (P[i][(jmax_local + 3 - 1 - 1) + 1] - P[i][(jmax_local + 3 - 1 - 1)]));
            updateMax(vMax, V[i][jmax_local + 3 - 1]);
        }
    }
