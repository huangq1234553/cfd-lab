  ...
  for(i = 1; i <= imax; i++) {
    for(j = 1; j<=jmax; j++) {
      ...
      if(isFluid(C)) {
          P[i][j] = (1.0 - omg) * P[i][j]
                    + coeff *
                      ((P[i + 1][j] + P[i - 1][j]) / (dx * dx) + (P[i][j + 1] + P[i][j - 1]) / (dy * dy) - RS[i][j]);
      }
    }
  }
  ...
  /* set boundary values on obstacle interface */
    for(i = 1; i <= imax; i++) {
        for(j = 1; j<=jmax; j++) {
            ...
            if (isObstacle(C)){
                if(isCorner(C)){
                    P[i][j] = (P[i + isNeighbourObstacle(C,LEFT)-isNeighbourObstacle(C,RIGHT)][j] + P[i][j + isNeighbourObstacle(C,BOT) - isNeighbourObstacle(C,TOP)])/2;
                }
                else{
