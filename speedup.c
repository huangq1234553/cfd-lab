...
int main(int argc, char **argv)
{
    ...
    char* problem_seq = "Sequential";

    long startTimeSeq = getCurrentTimeMillis();
    if(my_rank == 0) {
    ...
    t = 0;
    n = 0;
    long startTimePar = getCurrentTimeMillis();
    
    init_parallel(iproc, jproc, imax, jmax, my_rank, &il, &ir, &jb, &jt, &rank_l, &rank_r, &rank_b, &rank_t,
                  &omg_i, &omg_j, num_proc);
    
    imax_local = ir - il, jmax_local = jt - jb;
...
// 	// simulation interval 0 to t_end
    currentOutputTime = 0; // For chosing when to output
    while (t < t_end)
    {
        // ensure boundary conditions for velocity
        // here we only need to set boundary values if local boundaries coincide with global boundaries
...
//            printf("[R%d] Global dt after allreduce: %f\n", my_rank, dt); //debug
    ...
    long endTimePar = getCurrentTimeMillis();
    MPI_Finalize();
//    logMsg("Min dt value used: %16e", mindt);
    if (my_rank == 0)
    {
        double seqTime = getTimeSpentSeconds(startTimeSeq, endTimeSeq);
        double parTime = getTimeSpentSeconds(startTimePar, endTimePar);
