...
static char *TEST_NAME = "getProcessNeighbours.3x2.test";
...
{
    // Now assume we are in a 3x2 grid, so let's check we have enough MPI processes
    assert(mpiNumProc >= 6);
    ...
    {
        case 0:
            setup(3, 2, 0, 0, MPI_PROC_NULL, 1, MPI_PROC_NULL, 3);
            break;
        case 1:
            setup(3, 2, 1, 0, 0, 2, MPI_PROC_NULL, 4);
            break;
        case 2:
            setup(3, 2, 2, 0, 1, MPI_PROC_NULL, MPI_PROC_NULL, 5);
            break;
        case 3:
            setup(3, 2, 0, 1, MPI_PROC_NULL, 4, 0, MPI_PROC_NULL);
            break;
        case 4:
            setup(3, 2, 1, 1, 3, 5, 1, MPI_PROC_NULL);
            break;
        case 5:
            setup(3, 2, 2, 1, 4, MPI_PROC_NULL, 2, MPI_PROC_NULL);
            break;
        default:
