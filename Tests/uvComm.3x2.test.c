...
static char *TEST_NAME = "uvComm.3x2.test";
...
static void setup(int mpiRank, int rank_l_, int rank_r_, int rank_b_, int rank_t_)
{
    ...
    init_matrix(U, 0, imax + 3, 0, jmax + 2, mpiRank*U_FACTOR); // Initialize with rank
    init_matrix(V, 0, imax + 2, 0, jmax + 3, mpiRank*V_FACTOR); // Initialize with rank
...
int uvCommTest32(int mpiRank, int mpiNumProc)
{
    ...
    {
        case 0:
            setup(mpiRank, MPI_PROC_NULL, 1, MPI_PROC_NULL, 3);
            break;
        case 1:
            setup(mpiRank, 0, 2, MPI_PROC_NULL, 4);
            break;
        case 2:
            setup(mpiRank, 1, MPI_PROC_NULL, MPI_PROC_NULL, 5);
            break;
        case 3:
            setup(mpiRank, MPI_PROC_NULL, 4, 0, MPI_PROC_NULL);
            break;
        case 4:
            setup(mpiRank, 3, 5, 1, MPI_PROC_NULL);
            break;
        case 5:
            setup(mpiRank, 4, MPI_PROC_NULL, 2, MPI_PROC_NULL);
            break;
        default:
    ...
    if (rank_l != MPI_PROC_NULL) //left
    {
        ...
        {
            failedTests += expectEqual(U[0][i], rank_l*U_FACTOR, TEST_NAME, "[R%d] Left ghost layer check failed: U[0][%d] = ",
                                       mpiRank, i);
            failedTests += expectEqual(V[0][i], rank_l*V_FACTOR, TEST_NAME, "[R%d] Left ghost layer check failed: V[0][%d] = ",
                                       mpiRank, i);
        }
    }
    ...
    if (rank_r != MPI_PROC_NULL) //right
    {
        for (int i = 1; i <= imax; ++i)
        {
            ...
            failedTests += expectEqual(U[imax + 3][i], rank_r*U_FACTOR, TEST_NAME,
                                       "[R%d] Right ghost layer check failed: U[imax+3][%d] = ", mpiRank, i);
            failedTests += expectEqual(V[imax + 2][i], rank_r*V_FACTOR, TEST_NAME,
    ...
    if (rank_b != MPI_PROC_NULL) //bottom
    {
        for (int i = 1; i <= imax; ++i)
        {
            failedTests += expectEqual(U[i][0], rank_b*U_FACTOR, TEST_NAME, "[R%d] Bottom ghost layer check failed: U[%d][0] = ",
                                       mpiRank, i);
            failedTests += expectEqual(V[i][0], rank_b*V_FACTOR, TEST_NAME, "[R%d] Bottom ghost layer check failed: V[%d][0] = ",
                                       mpiRank, i);
        }
    }
    ...
    if (rank_t != MPI_PROC_NULL) //top
    {
        for (int i = 1; i <= imax; ++i)
        {
            failedTests += expectEqual(U[i][jmax + 2], rank_t*U_FACTOR, TEST_NAME,
                                       "[R%d] Top ghost layer check failed: U[%d][jmax+2] = ", mpiRank, i);
            failedTests += expectEqual(V[i][jmax + 3], rank_t*V_FACTOR, TEST_NAME,
